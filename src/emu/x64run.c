...
x64emurun:
...
#endif
    {
        ...
        // instruction whose addr is in [PRIDROID_TRACE_LO, PRIDROID_TRACE_HI) with key regs, so a
        ...
        // default (no env => no-op). Env parsed once.
        {
            ...
            if(!rd_t_init){ rd_t_init=1; char* a=getenv("PRIDROID_TRACE_LO"); char* b=getenv("PRIDROID_TRACE_HI");
