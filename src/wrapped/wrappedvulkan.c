...
static uintptr_t rd_mono_pmip_addr(void) {
    ...
    if(GetGlobalSymbolStartEnd(my_context->maplib, "mono_pmip", &start, &end, NULL, -1, NULL, 0, NULL))
        addr = start;
    printf_log(LOG_NONE, "PRIDROID: mono_pmip guest addr=%p\n", (void*)addr);
    return addr;
}
...
static uintptr_t rd_gsym(const char* name) {
    ...
    if(GetGlobalSymbolStartEnd(my_context->maplib, name, &start, &end, NULL, -1, NULL, 0, NULL))
        return start;
    printf_log(LOG_NONE, "PRIDROID: gsym MISSING %s\n", name);
    return 0;
}
...
static void rd_dump_longevent(void) {
    ...
    if(!f_rootdom || !f_imgload || !f_clsname || !f_fldname || !f_vtable || !f_sget) {
        printf_log(LOG_NONE, "PRIDROID: LONGEVENT probe — missing mono API, abort\n");
        return;
    }
    uint64_t domain = RunFunction(f_rootdom, 0);
    uint64_t image  = RunFunction(f_imgload, 1, (uint64_t)(uintptr_t)"Assembly-CSharp");
    if(!image) { printf_log(LOG_NONE, "PRIDROID: LONGEVENT — Assembly-CSharp image not loaded\n"); return; }
    uint64_t klass  = RunFunction(f_clsname, 3, image, (uint64_t)(uintptr_t)"Verse",
                                  (uint64_t)(uintptr_t)"LongEventHandler");
    if(!klass) { printf_log(LOG_NONE, "PRIDROID: LONGEVENT — class Verse.LongEventHandler not found\n"); return; }
    uint64_t vt = RunFunction(f_vtable, 2, domain, klass);
    printf_log(LOG_NONE, "PRIDROID: LONGEVENT domain=%p image=%p klass=%p vt=%p\n",
    ...
    for(int i = 0; fields[i]; ++i) {
        ...
        if(!fld) { printf_log(LOG_NONE, "PRIDROID: LE field %s = <not found>\n", fields[i]); continue; }
        void* val = NULL;
        ...
        printf_log(LOG_NONE, "PRIDROID: LE %s = %p (%s)\n", fields[i], val, rd_ptr_ok(val) ? cn : "null");
    ...
    if(rd_ptr_ok(currentEvent) && f_objcls && f_fldname && f_fget) {
        ...
        for(int i = 0; strs[i]; ++i) {
            ...
            printf_log(LOG_NONE, "PRIDROID: LE currentEvent.%s = \"%s\"\n", strs[i], buf);
        }
        ...
        for(int i = 0; bools[i]; ++i) {
            ...
            printf_log(LOG_NONE, "PRIDROID: LE currentEvent.%s = %d\n", bools[i], (int)b);
        }
        ...
        if(afld) {
            ...
            if(rd_ptr_ok(act)) {
                ...
                printf_log(LOG_NONE, "PRIDROID: LE currentEvent.eventAction.method = %s (delegate=%p method=%p)\n",
                           mn, act, method);
            } else {
                printf_log(LOG_NONE, "PRIDROID: LE currentEvent.eventAction = null\n");
            }
        }
        ...
        if(efld) {
            ...
            if(rd_ptr_ok(exc)) {
                ...
                printf_log(LOG_NONE, "PRIDROID: LE currentEvent.exception = %s : \"%s\"\n", ecn, msg);
            } else {
                printf_log(LOG_NONE, "PRIDROID: LE currentEvent.exception = null\n");
            }
        }
    }
    printf_log(LOG_NONE, "PRIDROID: LONGEVENT dump done\n");
}
...
static void rd_dump_unity_event_current(void)
{
    ...
    if(!f_thread_current || !f_imgload || !f_clsname || !f_method || !f_invoke || !f_unbox) {
        printf_log(LOG_NONE, "PRIDROID: UNITYEVENT probe missing mono API\n");
        return;
    }
    uint64_t mono_thread = RunFunction(f_thread_current, 0);
    if(!mono_thread) {
        printf_log(LOG_NONE, "PRIDROID: UNITYEVENT no attached Mono thread at vk bind site\n");
        return;
    }
    ...
    if(!image) {
        printf_log(LOG_NONE, "PRIDROID: UNITYEVENT UnityEngine image not loaded\n");
        return;
    }
    uint64_t klass = RunFunction(f_clsname, 3, image, (uint64_t)(uintptr_t)"UnityEngine",
                                 (uint64_t)(uintptr_t)"Event");
    if(!klass) {
        printf_log(LOG_NONE, "PRIDROID: UNITYEVENT UnityEngine.Event class not found (image=%p)\n",
                   (void*)image);
        return;
    }
    ...
    if(!m_current || !m_type) {
        printf_log(LOG_NONE, "PRIDROID: UNITYEVENT missing get_current/get_type methods current=%p type=%p\n",
    ...
    if(exc || !rd_ptr_ok((void*)(uintptr_t)current)) {
        printf_log(LOG_NONE, "PRIDROID: UNITYEVENT current=%p exc=%p\n",
                   (void*)current, exc);
        return;
    }
    void* type_exc = NULL;
    ...
    printf_log(LOG_NONE,
               "PRIDROID: UNITYEVENT current=%p type=%d(%s) rawType=%d(%s) cmd=\"%s\" exc(type/raw/cmd)=%p/%p/%p\n",
...
extern __attribute__((weak)) void* pridroid_get_native_window(void);
...
static void rd_dump_display_state(void) {
    ...
    printf_log(LOG_NONE, "PRIDROID: DISPLAY Screen=%dx%d fullScreen=%d displays.Length=%d | Display.main active=%d system=%dx%d rendering=%dx%d\n",
...
static void rd_dump_frame_summary(const char* where) {
    void* win = (&pridroid_get_native_window && pridroid_get_native_window) ? pridroid_get_native_window() : (void*)-1;
    printf_log(LOG_NONE, "PRIDROID: FRAME[%s] Time.frameCount=%d nativeWin=%p | CB begin=%d end=%d reset=%d | "
...
static void rd_dump_threads(void) {
    ...
    if(!d) { printf_log(LOG_NONE, "PRIDROID: THREADS — cannot open /proc/self/task\n"); return; }
    struct dirent* e;
    ...
    while((e = readdir(d))) {
        ...
        if(st == 'R' || total <= 6)
            printf_log(LOG_NONE, "PRIDROID: THREAD tid=%s state=%c comm=%s\n", e->d_name, st, comm);
    }
    closedir(d);
    printf_log(LOG_NONE, "PRIDROID: THREADS total=%d running=%d sleeping=%d other=%d\n",
...
{
    // PriDroid: trace every vk* the guest resolves (1.6 surface-bail hunt) — remove later.
    if(rname && strstr(rname, "Surface"))
        printf_log(LOG_NONE, "PRIDROID: resolve \"%s\" my=%p host=%p\n", rname, symbol, fnc);
    // get wrapper
...
EXPORT void* my_vkGetDeviceProcAddr(x64emu_t* emu, void* device, void* name)
{
    ...
    // PriDroid: swapchain-path telemetry — Unity silently drops the surface if these resolve NULL
    ...
    if(rname && (!strcmp(rname,"vkCreateSwapchainKHR") || !strcmp(rname,"vkQueuePresentKHR") ||
        ...
        printf_log(LOG_NONE, "PRIDROID: vkGetDeviceProcAddr(\"%s\") host=%p\n", rname, fnc?fnc:symbol);
    if(!symbol) {
        printf_dlsym_prefix(0, LOG_DEBUG, "%p\n", NULL);
        return NULL;    // easy
    }
    ...
    if(rname && !strcmp(rname, "vkQueuePresentKHR")) {
        ...
        printf_log(LOG_NONE, "PRIDROID: guest bridge vkQueuePresentKHR=%p\n", guestsym);
    } else if(rname && !strcmp(rname, "vkAcquireNextImageKHR")) {
        rd_acq_bridge = guestsym;
        printf_log(LOG_NONE, "PRIDROID: guest bridge vkAcquireNextImageKHR=%p\n", guestsym);
    }
    return guestsym;
}
...
EXPORT void* my_vkGetInstanceProcAddr(x64emu_t* emu, void* instance, void* name)
{
   ...
   printf_dlsym(LOG_DEBUG, "Calling my_vkGetInstanceProcAddr[%p](%p, \"%s\") => ", getprocaddr, instance, rname);
    ...
    void* fnc = NULL;
#ifdef ANDROID
    // PriDroid: the Android-only host driver has no X11 WSI entry points; resolve them to our
    ...
    if(!symbol && rname && is_my && (
        ...
        if(mysym) {
            printf_log(LOG_INFO, "PRIDROID: emulated X11 WSI \"%s\"\n", rname);
...
#ifdef ANDROID
// PriDroid: Android has no libvulkan.so.1 — the host library is "libvulkan.so"
// (the system loader). Our libpridroidlinker interposes dlopen process-wide and
...
// PriDroid: device-memory telemetry for the load-phase Vulkan OOM (rimworld_16_port session 5).
...
EXPORT int my_vkAllocateMemory(x64emu_t* emu, void* device, void* pAllocateInfo, my_VkAllocationCallbacks_t* pAllocator, void* p)
{
    ...
    if(ret==0 && pAllocateInfo) {
        ...
        if(rd_mem_allocs <= 20 || (rd_mem_allocs % 100)==0)
            printf_log(LOG_NONE, "PRIDROID: vkAllocateMemory #%lld size=%lldKB type=%u frees=%lld totalMB~%lld\n",
                       rd_mem_allocs, size/1024, type, rd_mem_frees, rd_mem_bytes/(1024*1024));
    } else if(ret!=0)
        printf_log(LOG_NONE, "PRIDROID: vkAllocateMemory FAILED ret=%d size=%lld (allocs=%lld frees=%lld totalMB=%lld)\n",
...
EXPORT int my_vkCreateDevice(x64emu_t* emu, void* physdev, void* pCreateInfo, my_VkAllocationCallbacks_t* pAllocator, void* pDevice)
{
    ...
    // PriDroid: log the device extensions — a device without VK_KHR_swapchain explains the
    ...
    if(pCreateInfo) {
        ...
        printf_log(LOG_NONE, "PRIDROID: vkCreateDevice ret=%d ext_count=%u\n", ret, n);
        for(uint32_t i=0; i<n && i<40 && ext; ++i)
            printf_log(LOG_NONE, "PRIDROID:   dev-ext[%u]=%s\n", i, ext[i]);
    }
    return ret;
}
...
EXPORT int my_vkCreateImage(x64emu_t* emu, void* device, void* pCreateInfo, my_VkAllocationCallbacks_t* pAllocator, void* p)
{
    ...
    if(ret == 0 && pCreateInfo && p) {
        ...
        if((usage & 0x10) && w >= 640 && h >= 360) {   // any largish COLOR_ATTACHMENT — log where Unity renders
            ...
            if(rd_col_logged++ < 30)
                printf_log(LOG_NONE, "PRIDROID: COLORIMG=%p fmt=%u usage=0x%x %ux%u\n", *(void**)p, fmt, usage, w, h);
...
{
    // PriDroid GLES pivot: deny Vulkan to Unity so its auto graphics-API selection
    ...
    if(getenv("PRIDROID_FORCE_GLES")) {
        printf_log(LOG_INFO, "PRIDROID: PRIDROID_FORCE_GLES set -> vkCreateInstance returns VK_ERROR_INCOMPATIBLE_DRIVER (force GLES fallback)\n");
...
#ifdef ANDROID
    // PriDroid: if the guest enables the X11 WSI extensions (we advertise them in
    ...
    if(rd_ci && rd_ci->ppExt && rd_ci->nExt) {
        ...
        if(has_x11) {
            ...
            printf_log(LOG_INFO, "PRIDROID: vkCreateInstance X11 WSI -> android_surface (%u -> %u ext)\n", rd_old_n, n);
        }
    }
#endif
...
// PriDroid: swapchain/present telemetry for the 1.6 GPU-memory-leak hunt (see rimworld_16_port).
...
EXPORT int my_vkCreateSwapchainKHR(x64emu_t* emu, void* device, void* pCreateInfo, my_VkAllocationCallbacks_t* pAllocator, void* pSwapchain)
{
    ...
    printf_log(LOG_NONE, "PRIDROID: vkCreateSwapchainKHR ENTER extent=%ux%u fmt=%d minImages=%u mode=%d clipped=%u preTransform=0x%x old=%p fnc=%p\n",
    ...
    {
        printf_log(LOG_NONE, "PRIDROID: vkCreateSwapchainKHR EXIT #%d ret=%d extent=%ux%u fmt=%d minImages=%u old=%p -> %p\n",
                   rd_swap_creates, ret,
                   ci?*(uint32_t*)(ci+44):0, ci?*(uint32_t*)(ci+48):0,
                   ci?*(int*)(ci+36):0, ci?*(uint32_t*)(ci+32):0,
                   ci?*(void**)(ci+96):NULL,
...
#ifdef ANDROID
// PriDroid: the host Vulkan driver (Turnip) has NO X11 WSI — only Android WSI. When the guest
...
{
    if(!(&pridroid_get_native_window) || !pridroid_get_native_window)
        return 1;
    void* win = pridroid_get_native_window();
    printf_log(LOG_INFO, "PRIDROID: rd_android_surface win=%p\n", win);
    if(!win)
        return 1;
    ...
    printf_log(LOG_INFO, "PRIDROID: createAndroid=%p\n", createAndroid);
    if(!createAndroid)
        return 1;
    rd_VkAndroidSurfaceCreateInfoKHR_t aci = { 1000008000, NULL, 0, win };
    my_VkAllocationCallbacks_t my_alloc;
    printf_log(LOG_INFO, "PRIDROID: vkCreateAndroidSurfaceKHR ENTER tid=%d win=%p\n", GetTID(), win);
    int ret = createAndroid(instance, &aci, find_VkAllocationCallbacks(&my_alloc, pAllocator), pSurface);
    printf_log(LOG_NONE, "PRIDROID: vkCreateAndroidSurfaceKHR EXIT tid=%d ret=%d surface=%p\n",
...
{
    if(info)
        printf_log(LOG_NONE, "PRIDROID: XcbSurface XID=0x%x conn=%p\n",
    ...
    if(!fnc) {
        printf_log(LOG_NONE, "PRIDROID: vkCreateXcbSurfaceKHR — no host fn and no ANativeWindow (screen off?)\n");
...
{
    printf_log(LOG_INFO, "PRIDROID: vkCreateXlibSurfaceKHR(instance=%p)\n", instance);
    // Which X11 window does Unity present to? (expert Q: surface XID vs the window we heal)
    if(info)
        printf_log(LOG_NONE, "PRIDROID: XlibSurface XID=0x%lx dpy=%p\n",
    ...
    if(!fnc) {
        printf_log(LOG_NONE, "PRIDROID: vkCreateXlibSurfaceKHR — no host fn and no ANativeWindow (screen off?)\n");
        return -3; // VK_ERROR_INITIALIZATION_FAILED — never jump to NULL
    }
    return fnc(instance, info, find_VkAllocationCallbacks(&my_alloc, pAllocator), pSurface);
}

// PriDroid: on Android the host driver has no X11 WSI, so these queries have no host function.
...
{
    (void)emu;
    printf_log(LOG_INFO, "PRIDROID: XlibPresentationSupport(dev=%p qf=%u)\n", dev, queueFamily);
#ifdef ANDROID
    if(&pridroid_get_native_window)
        return 1;
#endif
...
// PriDroid: inflate VK_EXT_memory_budget numbers — Unity 2022 self-declares
...
// PriDroid: the System driver aborts INSIDE host vkCmdBindDescriptorSets on the first real frame
...
static void rd_vkslot_scan(void) {
    ...
    if(!delta) {
        ...
        if(!delta) { printf_log(LOG_NONE, "PRIDROID: VKSLOT no UnityPlayer elf\n"); return; }
    }
    ...
    for(uintptr_t a = (uintptr_t)delta + 0x1ed7000; a < (uintptr_t)delta + 0x2200000; a += 8) {
        ...
        if(v && (v == rd_qp_bridge || v == rd_acq_bridge))
            printf_log(LOG_NONE, "PRIDROID: VKSLOT %s at UnityPlayer+0x%lx\n",
                       (v == rd_qp_bridge) ? "QueuePresent" : "AcquireNextImage",
                       (unsigned long)(a - (uintptr_t)delta));
    }
    printf_log(LOG_NONE, "PRIDROID: VKSLOT scan done\n");
}
...
EXPORT int my_vkBeginCommandBuffer(x64emu_t* emu, void* cmdbuf, void* info)
{
    ...
    if(rd_cb_begin <= 8 || (rd_cb_begin % 200) == 0)
        printf_log(LOG_NONE, "PRIDROID: CB Begin#%d cmd=%p (end=%d reset=%d rp=%d)\n",
...
EXPORT int my_vkEndCommandBuffer(x64emu_t* emu, void* cmdbuf)
{
    ...
    if(rd_cb_end <= 8 || (rd_cb_end % 200) == 0)
        printf_log(LOG_NONE, "PRIDROID: CB End#%d cmd=%p (begin=%d reset=%d rp=%d)\n",
...
EXPORT int my_vkResetCommandBuffer(x64emu_t* emu, void* cmdbuf, uint32_t flags)
{
    ...
    if(rd_cb_reset <= 8 || (rd_cb_reset % 200) == 0)
        printf_log(LOG_NONE, "PRIDROID: CB Reset#%d cmd=%p\n", rd_cb_reset, cmdbuf);
...
EXPORT void my_vkCmdBeginRenderPass(x64emu_t* emu, void* cmdbuf, void* info, uint32_t contents)
{
    ...
    if(rd_rp_begin <= 8 || (rd_rp_begin % 1000) == 0)
        printf_log(LOG_NONE, "PRIDROID: RenderPass Begin#%d cmd=%p\n", rd_rp_begin, cmdbuf);
...
EXPORT void my_vkCmdEndRenderPass(x64emu_t* emu, void* cmdbuf)
{
    ...
    if(rd_rp_end <= 8 || (rd_rp_end % 1000) == 0)
        printf_log(LOG_NONE, "PRIDROID: RenderPass End#%d cmd=%p\n", rd_rp_end, cmdbuf);
    fnc(cmdbuf);
}
...
EXPORT void my_vkCmdBeginRendering(x64emu_t* emu, void* cmdbuf, void* info)
{
    ...
    if(rd_dynr_begin <= 4 || (rd_dynr_begin % 2000) == 0)
        printf_log(LOG_NONE, "PRIDROID: DynRendering Begin#%d cmd=%p\n", rd_dynr_begin, cmdbuf);
    fnc(cmdbuf, info);
}
...
static void rd_synthetic_present(void) {
    void* win = (&pridroid_get_native_window && pridroid_get_native_window) ? pridroid_get_native_window() : (void*)-1;
    printf_log(LOG_NONE, "PRIDROID: SYNPRESENT begin nativeWin=%p dev=%p swap=%p queue=%p imgs=%u qf=%u\n",
    ...
    if(!rd_h_device || !rd_h_swapchain || !rd_h_queue || !rd_h_image_count) {
        printf_log(LOG_NONE, "PRIDROID: SYNPRESENT missing handles — abort\n");
        return;
    }
    ...
    if(!CreateFence||!Acquire||!WaitFences||!ResetFences||!CreatePool||!AllocCmd||!BeginCmd||!EndCmd||!Barrier||!Clear||!Submit||!Present){
        printf_log(LOG_NONE, "PRIDROID: SYNPRESENT a host fn is NULL — abort\n"); return;
    }
    ...
    printf_log(LOG_NONE, "PRIDROID: SYNPRESENT acquire ret=%d idx=%u\n", r, idx);
    if(r != 0 && r != 1000001003) { printf_log(LOG_NONE, "PRIDROID: SYNPRESENT acquire failed\n"); return; }
    void* fences[1] = { fence };
    WaitFences(dev, 1, fences, 1, 2000000000ULL); ResetFences(dev, 1, fences);
    if(idx >= rd_h_image_count) { printf_log(LOG_NONE, "PRIDROID: SYNPRESENT idx OOB\n"); return; }
    ...
    printf_log(LOG_NONE, "PRIDROID: SYNPRESENT pool=%p cmd=%p\n", pool, cmd);
    if(!pool || !cmd) return;
    ...
    printf_log(LOG_NONE, "PRIDROID: SYNPRESENT presenting Unity's swapchain image0=%p directly (idx 0)\n", img0);
    ...
    printf_log(LOG_NONE, "PRIDROID: SYNPRESENT submit ret=%d\n", r);
    ...
    printf_log(LOG_NONE, "PRIDROID: SYNPRESENT *** vkQueuePresentKHR(image0) ret=%d *** RimWorld frame should show if it renders to swapchain img0\n", r);
}
        ...
        uint32_t firstSet, uint32_t setCount, void** pSets, uint32_t dynCount, void* pDynOffs)
{
    ...
    if(rd_bind_calls <= 12 || (rd_bind_calls % 500) == 0)
        printf_log(LOG_NONE, "PRIDROID: BindDescriptorSets #%d cmd=%p bp=%u layout=%p first=%u n=%u sets[0]=%p dyn=%u\n",
    ...
    if(rd_bind_calls == 1200 || rd_bind_calls == 2500) {
        printf_log(LOG_NONE, "PRIDROID: === LONGEVENT sample at bind #%d ===\n", rd_bind_calls);
        rd_dump_longevent();
    ...
    if(rd_skip && rd_bind_calls > 15000) {
        if(rd_bind_calls == 15001)
            printf_log(LOG_NONE, "PRIDROID: SURVIVAL — skipping host binds from #15001 to keep process alive\n");
        if((rd_bind_calls % 200000) == 0) {
            printf_log(LOG_NONE, "PRIDROID: === SURVIVAL re-sample at bind #%d ===\n", rd_bind_calls);
            rd_dump_longevent();
            rd_dump_unity_event_current();
            rd_dump_threads();
            char where[64];
...
static void rd_patch_memory_heaps(void* pMemProps, const char* who)
{
    ...
    for(uint32_t i=0; i<heapCount && i<16; ++i) {
        ...
        printf_log(LOG_NONE, "PRIDROID: heap[%u] size=%lldMB flags=%u (%s)%s\n",
...
static void rd_patch_memory_budget(void* pProps, const char* who)
{
    ...
    while(p) {
        ...
        if(sType == 1000237000) {  // VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_MEMORY_BUDGET_PROPERTIES_EXT
            ...
            printf_log(LOG_NONE, "PRIDROID: inflated memory budget (%s)\n", who);
        }
        p = *(char**)(p + 8);
    }
}
...
// PriDroid: hide VK_EXT_memory_budget from the guest — Unity 2022 self-declares
    ...
    if(ret>=0 && pProperties && pCount) {
        ...
        for(uint32_t i=0; i<*pCount; ++i)
            if(!strcmp(arr[i].name, "VK_EXT_memory_budget")) {
                ...
                printf_log(LOG_NONE, "PRIDROID: hid VK_EXT_memory_budget from the guest\n");
            }
    }
#endif
    return ret;
...
// PriDroid: advertise the X11 WSI instance extensions on top of the Android-only host driver.
...
#ifdef ANDROID
    if(!pLayerName && (&pridroid_get_native_window)) {
        typedef struct { char name[256]; uint32_t spec; } rd_VkExtProps_t;
...
EXPORT void my_vkDestroySwapchainKHR(x64emu_t* emu, void* device, void* swapchain, my_VkAllocationCallbacks_t* pAllocator)
{
    ...
    ++rd_swap_destroys;
    printf_log(LOG_NONE, "PRIDROID: vkDestroySwapchainKHR #%d handle=%p (creates=%d)\n",
...
EXPORT int my_vkGetSwapchainImagesKHR(x64emu_t* emu, void* device, void* swapchain, uint32_t* pCount, void* pImages)
{
    ...
    printf_log(LOG_NONE, "PRIDROID: vkGetSwapchainImagesKHR ENTER #%d swap=%p query=%d cap=%u fnc=%p\n",
               n, swapchain, pImages?1:0, pCount?*pCount:0, fnc);
    int ret = fnc(device, swapchain, pCount, pImages);
    printf_log(LOG_NONE, "PRIDROID: vkGetSwapchainImagesKHR EXIT #%d ret=%d count=%u\n",
                                    ...
                                    void* semaphore, void* fence, uint32_t* pImageIndex)
{
    ...
    int n = ++rd_acquires;
    if(n <= 10 || (n % 300) == 0)
        printf_log(LOG_NONE, "PRIDROID: vkAcquireNextImageKHR ENTER #%d swap=%p timeout=%llu sem=%p fence=%p fnc=%p\n",
    ...
    if(n <= 3) {
        ...
        printf_log(LOG_NONE, "PRIDROID: acquire backtrace #%d (%d frames):\n", n, fr);
        for(int i = 0; i < fr; ++i)
            printf_log(LOG_NONE, "PRIDROID:   ABT %s\n", syms ? syms[i] : "?");
        if(syms) box_free(syms);
    }
    int ret = fnc(device, swapchain, timeout, semaphore, fence, pImageIndex);
    if(n <= 10 || ret != 0 || (n % 300) == 0)
        printf_log(LOG_NONE, "PRIDROID: vkAcquireNextImageKHR EXIT #%d ret=%d image=%u\n",
...
EXPORT int my_vkAcquireNextImage2KHR(x64emu_t* emu, void* device, void* pAcquireInfo, uint32_t* pImageIndex)
{
    ...
    int n = ++rd_acquires;
    printf_log(LOG_NONE, "PRIDROID: vkAcquireNextImage2KHR ENTER #%d info=%p fnc=%p\n", n, pAcquireInfo, fnc);
    int ret = fnc(device, pAcquireInfo, pImageIndex);
    printf_log(LOG_NONE, "PRIDROID: vkAcquireNextImage2KHR EXIT #%d ret=%d image=%u\n",
               n, ret, pImageIndex?*pImageIndex:~0u);
    return ret;
}
...
EXPORT int my_vkQueueSubmit(x64emu_t* emu, void* queue, uint32_t submitCount, void* pSubmits, void* fence)
{
    ...
    if(n <= 10 || (n % 300) == 0) {
        ...
        printf_log(LOG_NONE, "PRIDROID: vkQueueSubmit ENTER #%d submits=%u wait=%u(%p) cmd=%u(%p) signal=%u(%p) fence=%p fnc=%p\n",
                   n, submitCount, waits, wait0, cmds, cmd0, signals, signal0, fence, fnc);
    }
    int ret = fnc(queue, submitCount, pSubmits, fence);
    if(n <= 10 || ret != 0 || (n % 300) == 0)
        printf_log(LOG_NONE, "PRIDROID: vkQueueSubmit EXIT #%d ret=%d\n", n, ret);
    return ret;
}
                              ...
                              uint32_t waitAll, uint64_t timeout)
{
    ...
    if(n <= 30 || (n % 300) == 0)
        printf_log(LOG_NONE, "PRIDROID: vkWaitForFences ENTER #%d count=%u first=%p all=%u timeout=%llu fnc=%p\n",
    ...
    if(n <= 30 || (ret != 0 && ret != 2) || (n % 300) == 0)
        printf_log(LOG_NONE, "PRIDROID: vkWaitForFences EXIT #%d ret=%d\n", n, ret);
    return ret;
}
...
EXPORT int my_vkGetFenceStatus(x64emu_t* emu, void* device, void* fence)
{
    ...
    if(n <= 30 || (ret != 0 && ret != 1) || (n % 300) == 0)
        printf_log(LOG_NONE, "PRIDROID: vkGetFenceStatus #%d fence=%p ret=%d\n", n, fence, ret);
    return ret;
}
...
EXPORT int my_vkResetFences(x64emu_t* emu, void* device, uint32_t count, void* pFences)
{
    ...
    if(n <= 30 || ret != 0 || (n % 300) == 0)
        printf_log(LOG_NONE, "PRIDROID: vkResetFences #%d count=%u first=%p ret=%d\n",
                   n, count, fence0, ret);
    return ret;
}
...
EXPORT int my_vkQueueWaitIdle(x64emu_t* emu, void* queue)
{
    ...
    printf_log(LOG_NONE, "PRIDROID: vkQueueWaitIdle ENTER queue=%p fnc=%p\n", queue, fnc);
    int ret = fnc(queue);
    printf_log(LOG_NONE, "PRIDROID: vkQueueWaitIdle EXIT ret=%d\n", ret);
    return ret;
}
...
EXPORT int my_vkDeviceWaitIdle(x64emu_t* emu, void* device)
{
    ...
    printf_log(LOG_NONE, "PRIDROID: vkDeviceWaitIdle ENTER device=%p fnc=%p\n", device, fnc);
    int ret = fnc(device);
    printf_log(LOG_NONE, "PRIDROID: vkDeviceWaitIdle EXIT ret=%d\n", ret);
    return ret;
}
...
EXPORT int my_vkQueuePresentKHR(x64emu_t* emu, void* queue, void* pPresentInfo)
{
    ...
    if(n <= 10 || (n % 300) == 0) {
        ...
        printf_log(LOG_NONE, "PRIDROID: vkQueuePresentKHR ENTER #%d count=%u image=%u fnc=%p\n",
                   n, count, image, fnc);
    }
    int ret = fnc(queue, pPresentInfo);
    if(n <= 10 || ret != 0 || (n % 300) == 0)
        printf_log(LOG_NONE, "PRIDROID: vkQueuePresentKHR EXIT #%d ret=%d\n", n, ret);
    return ret;
}
...
EXPORT uint32_t my_vkGetPhysicalDeviceSurfaceSupportKHR(x64emu_t* emu, void* dev, uint32_t qf, void* surface, void* pSupported)
{
    ...
    printf_log(LOG_NONE, "PRIDROID: SurfaceSupport ENTER qf=%u surface=%p fnc=%p\n", qf, surface, fnc);
    if(!fnc) return (uint32_t)-3;
    uint32_t ret = fnc(dev, qf, surface, pSupported);
    printf_log(LOG_NONE, "PRIDROID: SurfaceSupport ret=%d supported=%u\n",
...
EXPORT int my_vkGetPhysicalDeviceSurfaceFormatsKHR(x64emu_t* emu, void* dev, void* surface, uint32_t* pCount, void* pFormats)
{
    ...
    printf_log(LOG_NONE, "PRIDROID: SurfaceFormats surface=%p ret=%d count=%u query=%d\n",
...
EXPORT int my_vkGetPhysicalDeviceSurfaceCapabilitiesKHR(x64emu_t* emu, void* dev, void* surface, void* pCaps)
{
    ...
    printf_log(LOG_NONE, "PRIDROID: SurfaceCaps ENTER surface=%p fnc=%p\n", surface, fnc);
    if(!fnc) return -3;
    ...
    if(pCaps) {
        ...
        printf_log(LOG_NONE, "PRIDROID: SurfaceCaps ret=%d currentExtent=%ux%u min/maxImages=%u/%u supportedTransforms=0x%x currentTransform=0x%x\n",
        ...
        *(uint32_t*)((char*)pCaps+40) = 0x1;   // currentTransform    = IDENTITY
        if(curT != 0x1 || supT != 0x1)
            printf_log(LOG_NONE, "PRIDROID: SurfaceCaps SPOOFED transforms -> IDENTITY (was cur=0x%x sup=0x%x)\n", curT, supT);
    }
    return ret;
}
...
EXPORT int my_vkGetPhysicalDeviceSurfaceCapabilities2KHR(x64emu_t* emu, void* dev, void* pSurfaceInfo, void* pCaps)
{
    ...
    if(pCaps) {
        ...
        *(uint32_t*)(c+36) = 0x1; *(uint32_t*)(c+40) = 0x1;
        printf_log(LOG_NONE, "PRIDROID: SurfaceCaps2 ret=%d currentTransform=0x%x sup=0x%x -> IDENTITY\n", ret, curT, supT);
    }
    return ret;
}
...
EXPORT void my_vkFreeMemory(x64emu_t* emu, void* device, void* mem, my_VkAllocationCallbacks_t* pAllocator)
{
    ...
    if(rd_mem_frees <= 20 || (rd_mem_frees % 100)==0)
        printf_log(LOG_NONE, "PRIDROID: vkFreeMemory #%lld allocs=%lld handle=%p\n",
...
EXPORT void my_vkDestroySurfaceKHR(x64emu_t* emu, void* instance, void* surface, my_VkAllocationCallbacks_t* pAllocator)
{
    ...
    printf_log(LOG_NONE, "PRIDROID: vkDestroySurfaceKHR(%p)\n", surface);
